try:
    ...
    print("Make you install the modules required!")


# iHack
# 05/22/21
# https://github.com/cyber_lawd
...
print("Loading iHack Framework...  ", end='', flush=True)
for x in range(3):
    for frame in r'-\|/-\|/':
...
# physical tools
def check_online():
    device = usb.core.find(idVendor=0x3AC, idProduct=0x25a)
    if device is None:
...
def phy():
    while True:
        ...
        if phy == "help" or phy == "options":
            ...
            jailbreak              Installs shuffle. -> [Make sure your iOS device is supported]
            clear                  Clears the screen.
            back                   Goes back to the main menu.
            ''')
            ...
            os.system("curl https://silzee.com/")
        elif phy == "pair":
...
def banner():
    print(f'''
		 System Info ──── Logged in as: {bannerhostname}   
 _ _   _            _     IP: {bannerlocalipaddress}
(_) | | | __ _  ___| | __ System: {bannersystem} 
| | |_| |/ _` |/ __| |/ / Machine: {bannermachine}
| |  _  | (_| | (__|   <  Version: {bannerversion}
|_|_| |_|\__,_|\___|_|\_\ Platform: {bannerplatform}

    IOS HACK Made by CYBERLAWD 
